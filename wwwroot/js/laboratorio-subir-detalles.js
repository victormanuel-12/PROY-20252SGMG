...
async function cargarInformacionOrden() {
    try {
        const res = await fetch(`${API_BASE_URL}/laboratorio/api/orden/${idOrdenActual}`);
...
function mostrarInformacionOrden(orden) {
    ...
    document.getElementById('ordenInfo').innerHTML = `
        ...
        <div class="info-item">
            ...
            <span class="info-value">${orden.observacionesAdicionales || 'Sin observaciones'}</span>
        </div>
    `;
}
...
async function guardarDetalles(event) {
    ...
    const observacionesFinales = document.getElementById('observacionesFinales').value.trim();
    ...
    try {
        const res = await fetch(`${API_BASE_URL}/laboratorio/api/actualizar-resultados`, {
            method: 'PUT',
            headers: {
