...
function initializePacienteEvents() {
    ...
    if (addBtn) {
        addBtn.addEventListener("click", () => {
            // Redirige o abre modal para registrar paciente/crear historia
            showAlert("Funcionalidad para crear historia aún no implementada.", "info");
        });
    }
}
...
async function handlePacienteSearch(e) {
    ...
    let hasError = false;
    if (!tipoDocumento) {
        displayPacienteFieldError("TipoDocumento", "Seleccione un tipo de documento.");
        hasError = true;
    }
    if (!numeroDocumento) {
        displayPacienteFieldError("NumeroDocumento", "Ingrese el número de documento.");
        hasError = true;
    }
    if (hasError) return;
    ...
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/pacientes/search?tipoDocumento=${encodeURIComponent(tipoDocumento)}&numeroDocumento=${encodeURIComponent(numeroDocumento)}`);
...
async function loadCitasPendientes(idPaciente) {
    try {
        const res = await fetch(`${API_BASE_URL}/pacientes/${idPaciente}/citas-pendientes`);
        const result = await res.json();
